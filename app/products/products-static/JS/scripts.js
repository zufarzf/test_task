...
socket.on('update_products_category', function(data) {
    // data — это массив объектов [{id:..., name:...}, ...]
    $.each(data, function(index, category) {
        let existing = $("#categories-list").find(`.list-item[ data-id="${category.id}"]`);

        let template = `
        <div class="col list-item" data-id="${ category.id }">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="card-title-wrapper">
                        ...
                        <div class="dots-btn-wrapper">
                            ...
                            <div class="dropdown-menu dropdown-menu-end dots-btn-dropdown" data-popper-placement="bottom-end" style="position: absolute; inset: 0px 0px auto auto; margin: 0px; transform: translate(-15px, 0px);">
                                ...
                                <!-- item-->
                                <a href="${ category.edit_url }" class="dropdown-item notify-item">
                                ...
                                <!-- item-->
                                <a href="#" class="remove-category-btn dropdown-item notify-item"
									data-bs-toggle="modal"
									data-bs-target="#staticBackdrop"
									data-id="${ category.id }"    
									data-socket-name="remove-category"    
								>
									<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		if (category.remove) {
			...
			if (existing.length > 0) {
				...
				$("#categories-list").prepend(newItem);
			}
		}
    });
});
...
$(document).on('click', '.cancel-remove-category-btn', function () {
	item_data_for_remove.cat_id = null;
	item_data_for_remove.socket_name = null;
});

$(document).on('click', '.confirm-remove-category-btn', function () {
